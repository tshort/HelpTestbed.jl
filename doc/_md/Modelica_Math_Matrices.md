...
---------------
  ...
  ![image49](Modelica.Math.Matrices.is Solve a linear equality constrained
  ...
  ![image58](Modelica.Math.Matrices.he Return the real Schur form (rsf) S
  ...
  ![image68](Modelica.Math.Matrices.ba Return the Frobenius norm of a
  lanceS.png)                          matrix
  ...
  ![image77](Modelica.Math.Matrices.co Return solution of the
...
> #### See also
>
> [Matrices.rcond](Modelica_Math_Matrices.html#Modelica.Math.Matrices.rcond).
>
> ::
>
> Extends from
...
>        :]          
>
>   Real eps   0     If eps \> 0, the singular values are checked against eps;
>                    otherwise
>                    eps=max(size(A))\*norm(A)\*Modelica.Constants.eps is used
>   --------------------------------------------------------------------------
>
> ### Outputs
>
...
> * * * * *
>
> ![image102](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).conditionNumber
> ---------------------------------------------------------------------------------------------------------------------------------------------
>
...
> ratio of the largest to the smallest singular value of **A**. For more
> details, see
> [http://en.wikipedia.org/wiki/Condition\_number](http://en.wikipedia.org/wiki/Condition_number).
>
> #### Example
>
> >     A = [1, 2;
...
>   Real   A[:, :]           Input matrix
>
>   Real   p        2        Type of p-norm (only allowed: 1, 2 or
...
> * * * * *
>
> ![image103](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).rcond
> -----------------------------------------------------------------------------------------------------------------------------------
>
...
> [DGECON](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dgecon).
...
> #### See also
>
> [Matrices.conditionNumber](Modelica_Math_Matrices.html#Modelica.Math.Matrices.conditionNumber)
>
> ::
>
> Extends from
...
>   Type     Name           Default  Description
>   -------- -------------- -------- ----------------------------------------
>   Real     A[:, size(A,            Square real matrix
>            1)]                     
>
...
> * * * * *
>
> ![image104](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).norm
> ----------------------------------------------------------------------------------------------------------------------------------
>
...
> #### Description
>
> The function call "`Matrices.norm(A)`" returns the 2-norm of matrix A,
> i.e., the largest singular value of A. The function call
> "`Matrices.norm(A, p)`" returns the p-norm of matrix A. The only
> allowed values for p are
>
...
> #### See also
>
> [Matrices.frobeniusNorm](Modelica_Math_Matrices.html#Modelica.Math.Matrices.frobeniusNorm)
>
> ::
>
> Extends from
...
> * * * * *
>
> ![image105](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).frobeniusNorm
> -------------------------------------------------------------------------------------------------------------------------------------------
>
...
> * * * * *
>
> ![image106](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).nullSpace
> ---------------------------------------------------------------------------------------------------------------------------------------
>
...
>
>   Integer    nullity            Nullity, i.e., the dimension of the
>                                 nullspace
>   -------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> ![image107](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).exp
> ---------------------------------------------------------------------------------------------------------------------------------
>
...
> #### Description
>
> This function computes the exponential e^\*\*A\*\*T^ of matrix **A**,
> i.e.
>
> >     (AT)^2   (AT)^3
> >
...
> > >   ~ 2! 3!
> > >
> where e=2.71828..., **A** is an n x n matrix with real elements and T
> is a real number, e.g., the sampling time. **A** may be singular. With
> the exponential of a matrix it is, e.g., possible to compute the
> solution of a linear system of differential equations
>
...
> 3.  The matrix from (2) is approximated by explicitly performing the
...
> 4.  The resulting matrix is transformed back, by reverting the steps
>     of (2) and (1).
>
...
> exponential of a matrix. SIAM Review 20, pp. 801-836, 1979' or in the
> documentation of m-file expm2 in Matlab version 6
> ([http://www.MathWorks.com](http://www.MathWorks.com)) where it is
> stated that 'As a practical numerical method, this is often slow and
> inaccurate'. These statements are valid for a direct implementation of
> the Taylor series expansion, but *not* for the implementation variant
...
> * * * * *
>
> ![image108](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).integralExp
> -----------------------------------------------------------------------------------------------------------------------------------------
>
...
> transformation matrix *D*. This reduce the effort of following
> calculations. Afterwards the result have to be re-balanced by
> transformation D\*Atransf \*inv(D). Scaling halfen T k-times, until
> the norm of A\*T is less than 0.5. This garantees minumum rounding
> errors in the following series expansion. The re-scaling based on the
> equation exp(A\*2T) = exp(AT)\^2. The needed re-scaling formula for
> psi thus becomes:
>
>     Φ = Φ'*Φ'
>
> > I + A*Ψ = I + 2A*Ψ' + A\^2*Ψ'\^2 Ψ = A*Ψ'\^2 + 2\*Ψ'
>
> where psi' is the scaled result from the series expansion while psi is
> the re-scaled matrix.
>
...
> * * * * *
>
> ![image109](Modelica.Math.Matrices.balanceI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).integralExpT
> ------------------------------------------------------------------------------------------------------------------------------------------
>
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).continuousLyapunov
> -----------------------------------------------------------------------------------------------
>
...
> >     X*A + A'*X = C
>
> using the Schur method for Lyapunov equations proposed by Bartels and
> Stewart [1].
>
...
> >     Y*R' + R*Y = D
>
> with **R**=**U**'\***A'**\***U** is the real Schur form of **A**' and
> **D**=**U**'\***C**\***U** and **Y**=**U**'\***X**\***U** are the
> corresponding transformations of **C** and **X**. This problem is
> solved sequently for the 1x1 or 2x2 Schur blocks by exploiting the
> block triangular form of **R**. Finally the solution of the original
> problem is recovered as **X**=**U**\***Y**\***U**'. The boolean input
> "ATisSchur" indicates to omit the transformation to Schur in the case
> that **A**' has already Schur form.
>
> #### References
>
...
> #### See also
>
> [Matrices.continuousSylvester](Modelica_Math_Matrices.html#Modelica.Math.Matrices.continuousSylvester),
> [Matrices.discreteLyapunov](Modelica_Math_Matrices.html#Modelica.Math.Matrices.discreteLyapunov)
>
> ::
>
> ### Inputs
...
>   Type   Name             Default             Description
>   ------ ---------------- ------------------- -----------------------------
>   Real   A[:, size(A, 1)]                     Square matrix A in X\*A +
>                                               A'\*X = C
>
>   Real   C[size(A, 1),                        Square matrix C in X\*A +
>          size(A, 2)]                          A'\*X = C
>
>   Boolea ATisSchur        false               True if transpose(A) has
>   n                                           already real Schur form
>
>   Real   eps              Modelica.Math.Matri Tolerance eps
>                           ces.norm(...        
>   -------------------------------------------------------------------------
>
> ### Outputs
...
>   Type   Name                   Description
>   ------ ---------------------- ------------------------------------------
>   Real   X[size(A, 1), size(A,  Solution X of the Lyapunov equation X\*A +
>          2)]                    A'\*X = C
>   ------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).continuousSylvester
> ------------------------------------------------------------------------------------------------
>
...
> >     A*X + X*B = C.
>
> using the Schur method for Sylvester equations proposed by Bartels and
> Stewart [1].
>
> In a nutshell, the problem is reduced to the corresponding problem
>
...
> corresponding transformations of **C** and **X**. This problem is
> solved sequently by exploiting the block triangular form of **S** and
> **T**. Finally the solution of the original problem is recovered as
> **X**=**U**'\***Y**\***V**. The boolean inputs "AisSchur" and
> "BisSchur" indicate to omit one or both of the transformation to Schur
> in the case that **A** and/or **B** have already Schur form.
>
> The function applies LAPACK-routine DTRSYL. See
> [LAPACK.dtrsyl](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dtrsyl)
...
> #### See also
>
> [Matrices.discreteSylvester](Modelica_Math_Matrices.html#Modelica.Math.Matrices.discreteSylvester),
> [Matrices.continuousLyapunov](Modelica_Math_Matrices.html#Modelica.Math.Matrices.continuousLyapunov)
>
> ::
>
> ### Inputs
...
>
>   Real      C[size(A, 1), size(B,            Matrix C
>             2)]                              
>
>   Boolean   AisSchur               false     True if A has already real
>                                              Schur form
>
>   Boolean   BisSchur               false     True if B has already real
>                                              Schur form
>   --------------------------------------------------------------------------
>
> ### Outputs
...
>   Type    Name                     Description
>   ------- ------------------------ ----------------------------------------
>   Real    X[size(A, 1), size(B,    Solution of the continuous Sylvester
>           2)]                      equation
>   -------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).continuousRiccati
> ----------------------------------------------------------------------------------------------
>
> **Return solution X of the continuous-time algebraic Riccati equation
> A'\*X + X\*A - X\*B\*inv(R)\*B'\*X + Q = 0 (care)**
>
> ### Information
>
> ::
...
> >
> > > (X, alphaReal, alphaImag) = Matrices.continuousRiccati(A, B, R, Q,
> > > true);
>
> #### Description
>
...
> >     U = [U11, U12; U21, U22]
>
> with dimenstions according to **S**, the solution **X** is calculated
> by
>
> >     X*U11 = U21.
>
> With optional input `refinement=true` a subsequent iterative
> refinement based on Newton's method with exact line search is applied.
> See
...
> [dgehrd](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dgehrd)
> (to compute the upper Hessenberg matrix of **H**),
> [dorghr](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dorghr)
> (to calculate the orthogonal matrix from the elementary reflectors as
> returned from dgehrd),
> [dhseqr](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dhseqr)
> (to put transformed **H** to Schur form and to calculate the
> eigenvalues of the closed loop system) and
> [dtrsen](Modelica_Math_Matrices_LAPACK.html#Modelica.Math.Matrices.LAPACK.dtrsen)
...
>
>   Real     R[size(B, 2), size(B, identity(size(B,  Matrix R in CARE
>            2)]                   2))               
>
>   Real     Q[size(A, 1), size(A, identity(size(A,  Matrix Q in CARE
>            1)]                   1))               
>
>   Boolean  refine                false             True for subsequent
>                                                    refinement
>   -------------------------------------------------------------------------
>
> ### Outputs
...
>   Type   Name                   Description
>   ------ ---------------------- -------------------------------------------
>   Real   X[size(A, 1), size(A,  stabilizing solution of CARE
>          2)]                    
>
>   Real   alphaReal[size(H, 1)]  Real parts of
>                                 eigenvalue=alphaReal+i\*alphaImag
>
>   Real   alphaImag[size(H, 1)]  Imaginary parts of
>                                 eigenvalue=(alphaReal+i\*alphaImag
>   -------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).discreteLyapunov
> ---------------------------------------------------------------------------------------------
>
...
> >     A*X*B - X + Q = 0.
>
> The algorithm uses the Schur method for Lyapunov equations proposed by
> Bartels and Stewart [1].
>
> In a nutshell, the problem is reduced to the corresponding problem
>
> >     R*Y*R' + sgn*Y = D.
>
> with **R**=**U**'\***A'**\***U** is the the real Schur form of **A**'
> and **D**=**U**'\***C**\***U** and **Y**=**U**'\***X**\***U** are the
> corresponding transformations of **C** and **X**. This problem is
> solved sequently by exploiting the block triangular form of **R**.
> Finally the solution of the original problem is recovered as
> **X**=**U**\***Y**\***U**'. The boolean input "ATisSchur" indicates to
> omit the transformation to Schur in the case that **A**' has already
> Schur form.
>
> #### References
>
...
>   Type   Name             Default              Description
>   ------ ---------------- -------------------- ----------------------------
>   Real   A[:, size(A, 1)]                      Square matrix A in A'\*X\*A
>                                                + sgn\*X = C
>
>   Real   C[size(A, 1),                         Square matrix C in A'\*X\*A
>          size(A, 2)]                           + sgn\*X = C
>
>   Boolea ATisSchur        false                True if transpose(A) has
>   n                                            already real Schur form
>
>   Intege sgn              1                    Specifies the sign in
>   r                                            A'\*X\*A + sgn\*X = C
>
>   Real   eps              Matrices.norm(A,     Tolerance eps
>                           1)\*10\*Model...     
>   -------------------------------------------------------------------------
>
> ### Outputs
...
>   Type   Name                  Description
>   ------ --------------------- --------------------------------------------
>   Real   X[size(A, 1), size(A, Solution X of the Lyapunov equation A'\*X\*A
>          2)]                   + sgn\*X = C
>   -------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).discreteSylvester
> ----------------------------------------------------------------------------------------------
>
...
> method proposed by Golub et al [1]. For sgn = -1, the discrete
> Sylvester equation is also known as Stein equation:
>
> >     A*X*B - X + Q = 0.
>
> In a nutshell, the problem is reduced to the corresponding problem
...
> appropriate transformations of **C** and **X**. This problem is solved
> sequently by exploiting the specific forms of **S** and **H**. Finally
> the solution of the original problem is recovered as
> **X**=**U**'\***Y**\***V**. The boolean inputs "AisHess" and
> "BTisSchur" indicate to omit one or both of the transformation to
> Hessenberg form or Schur form repectively in the case that **A**
> and/or **B** have already Hessenberg form or Schur respectively.
>
> #### References
>
...
>   Type   Name             Default               Description
>   ------ ---------------- --------------------- ---------------------------
>   Real   A[:, size(A, 1)]                       Square matrix A in A\*X\*B
>                                                 + sgn\*X = C
>
>   Real   B[:, size(B, 1)]                       Square matrix B in A\*X\*B
>                                                 + sgn\*X = C
>
>   Real   C[size(A, 2),                          Rectangular matrix C in
>          size(B, 1)]                            A\*X\*B + sgn\*X = C
>
>   Boolea AisHess          false                 True if A has already
>   n                                             Hessenberg form
>
>   Boolea BTisSchur        false                 True if B' has already real
>   n                                             Schur form
>
>   Intege sgn              1                     Specifies the sign in
>   r                                             A\*X\*B + sgn\*X = C
>
>   Real   eps              Matrices.norm(A,      Tolerance
>                           1)\*10\*Model...      
>   -------------------------------------------------------------------------
>
> ### Outputs
...
>   Type   Name                 Description
>   ------ -------------------- ----------------------------------------------
>   Real   X[size(A, 2),        solution of the discrete Sylvester equation
>          size(B, 1)]          A\*X\*B + sgn\*X = C
>   --------------------------------------------------------------------------
>
> ### Modelica definition
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).discreteRiccati
> --------------------------------------------------------------------------------------------
>
> **Return solution of discrete-time algebraic Riccati equation A'\*X\*A
> - X - A'\*X\*B\*inv(R + B'\*X\*B)\*B'\*X\*A + Q = 0 (dare)**
>
> ### Information
>
> ::
...
> >
> > > (X, alphaReal, alphaImag) = Matrices.discreteRiccati(A, B, R, Q,
> > > true);
>
> #### Description
>
...
> > > G = B*R*B'
>
> has no eigenvalue on the unit circle and can be put to an ordered real
> Schur form
>
> >     U'*H*U = S = [S11, S12; 0, S22]
>
...
>
>   Real     R[size(B, 2), size(B, identity(size(B,  Matrix R in DARE
>            2)]                   2))               
>
>   Real     Q[size(A, 1), size(A, identity(size(A,  Matrix Q in DARE
>            1)]                   1))               
>
>   Boolean  refine                false             True for subsequent
>                                                    refinement
...
>   Type   Name                 Description
>   ------ -------------------- ----------------------------------------------
>   Real   X[size(A, 1),        orthogonal matrix of the Schur vectors
>          size(A, 2)]          associated to ordered rsf
>
>   Real   alphaReal[size(H,    Real part of eigenvalue=alphaReal+i\*alphaImag
>          1)]                  
>
>   Real   alphaImag[size(H,    Imaginary part of
>          1)]                  eigenvalue=(alphaReal+i\*alphaImag
>   --------------------------------------------------------------------------
>
> ### Modelica definition
>
...
> * * * * *
>
> ![image110](Modelica.Math.Matrices.sortI.png) [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).sort
> -------------------------------------------------------------------------------------------------------------------------------
>
...
> >
> > > (sorted\_M, indices) = Matrices.sort(M, sortRows=true,
> > > ascending=true);
>
> #### Description
>
...
> "sortRows" is **false**, the columns of the matrix are sorted. If the
> optional argument "ascending" is **false**, the rows or columns are
> sorted in descending order. In the optional second output argument,
> the indices of the sorted rows or columns with respect to the original
> matrix are given, such that
>
...
>
>   Boolean   sortRows   true      = true if rows are sorted, otherwise
>                                  columns
>
...
>   Type   Name                           Description
>   ------ ------------------------------ ----------------------------------
>   Real   sorted\_M[size(M, 1), size(M,  Sorted matrix
>          2)]                            
>
>   Intege indices[if sortRows then       sorted\_M = if sortRows then
>   r      size(M, 1) else size(M, 2)]    M[indices,:] else M[:,indices]
>   ------------------------------------------------------------------------
>
> ### Modelica definition
>
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).flipLeftRight
> ------------------------------------------------------------------------------------------
>
...
> **A\_flr** with flipped columns, i.e., **A\_flr**[:,i]=**A**[:,n-i+1],
> i=1,..., n.
>
> #### Example
>
...
> #### See also
>
> [Matrices.flipUpDown](Modelica_Math_Matrices.html#Modelica.Math.Matrices.flipUpDown)
>
> ::
>
> ### Inputs
...
> * * * * *
>
> [Modelica.Math.Matrices](Modelica_Math_Matrices.html#Modelica.Math.Matrices).flipUpDown
> ---------------------------------------------------------------------------------------
>
...
> #### Description
>
> Function **flipUpDown** computes from matrix **A** a matrix **A\_fud**
> with flipped rows, i.e., **A\_fud**[i,:]=**A**[n-i+1,:], i=1,..., n.
>
> #### Example
>
> >     A = [1, 2,  3;
...
> #### See also
>
> [Matrices.flipLeftRight](Modelica_Math_Matrices.html#Modelica.Math.Matrices.flipLeftRight)
>
> ::
>
> ### Inputs
...
> * * * * *
>
> [Automatically generated](http://www.3ds.com/) Fri Nov 12 16:31:48
> 2010.
